...
The HTTP API contract is implementable, but APM client consumption of package registries is currently behind an experimental flag. Users must run `apm experimental enable package-registry` before installing from `registries:` entries.
::::
...
- Versions are **case-sensitive** canonical semver strings. A leading `v` prefix is not part of semver and MUST be rejected.
...
|---|---|---|
| `read` | `GET /versions`, `GET /download`, `GET /search` | Coarse read access |
...
1. **Version is parseable semver** (matches `^\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?(?:\+[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$`).
...
- **v1.0** (initial) — `versions`, `download`, `publish`, `search`. tar.gz + zip both supported. RFC 7807 errors. Bearer auth. Immutable versions. No yank, no signing, no pre-release advertising — those are v2.
